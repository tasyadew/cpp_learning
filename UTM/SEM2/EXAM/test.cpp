    ...
    public:
        ...
        void print(){
            ...
            cout << left << setw(5) << "No" << setw(20) << "Product Name" 
                 << setw(15) << "Category" << setw(13) << "Quantity" 
                 << setw(20) << "Unit Price(RM)" << setw(10) << "Amount (RM)\n";
            
            double total = 0.0;
            for (int i = 0; i < numProduct; i++){
                cout << setw(5) << (i+1) << setw(20) << prod[i].getName()
                     << setw(18) << prod[i].getCategory()
                     << setw(13) << prod[i].getQuantity()
                     << setw(20) << fixed << setprecision(2) << prod[i].getPrice()
                     << setw(20) << fixed << setprecision(2) << prod[i].getPrice()*prod[i].getQuantity()
                     << endl;
                total += (prod[i].getPrice()*prod[i].getQuantity());
            } 

            cout << "\n\nTotal price = RM " << fixed << setprecision(2) << total;
        }
};
